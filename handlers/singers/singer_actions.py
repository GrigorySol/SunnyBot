...
def show_singers(message: Message):
    ...
    bot.send_message(message.from_user.id,
                     f"В хоре {amount} певунов.\n{show_singers_text}", reply_markup=search_choice)
...
def show_by_voice(message: Message):
    ...
    if not voice:
        bot.send_message(message.from_user.id, no_voice_text)
    bot.send_message(message.from_user.id, f"Вы поёте в {voice}.")
...
def show_all_singers(call: CallbackQuery):
    bot.send_message(call.from_user.id, show_all_singers_text)
