...
from misc.messages.singer_dictionary import show_singers_text, you_shell_not_pass_text, \
    show_all_singers_text, CANCELED, suit_name_exists_text, add_suit_photo_text, no_photo_text, \
    new_suit_added_text, enter_the_suit_name_text
from misc.messages.changes_dictionary import blacklist_text, empty_blacklist_text
from misc.messages.event_dictionary import set_event_date_text, to_add_text_tuple, \
    song_or_event_text, choose_location_text, enter_location_url_text
from misc.messages.song_dictionary import enter_the_song_name_text, add_sheets_or_sounds_text, song_name_exists_text
...
def add_menu_handler(message: Message):
    ...
    call_config = "add_new"
    data = []
    for item_type, event in enumerate(to_add_text_tuple):
        if not item_type:
            continue
...
def song_or_event(call: CallbackQuery):
    ...
    if item_type == "4":
        ...
        """Add suit"""
        msg = bot.send_message(call.message.chat.id, enter_the_suit_name_text)
...
def add_song_name(message: Message):
    ...
    if "/" in message.text:
        ...
        msg = bot.send_message(message.chat.id, add_sheets_or_sounds_text)
...
def add_suit_name(message: Message):
    ...
    if "/" in message.text:
        ...
        msg = bot.send_message(message.chat.id, suit_name_exists_text)
        bot.register_next_step_handler(msg, add_suit_name)

    else:
        msg = bot.send_message(message.chat.id, add_suit_photo_text)
...
def save_new_suit(message: Message, suit_name):
    ...
    if "/" in message.text:
        ...
        print(message.photo)
        bot.register_next_step_handler(no_photo_text, save_new_suit, suit_name)

    else:
        db_singer.add_suit(suit_name, photo_file_id)
        bot.send_message(message.chat.id, new_suit_added_text)
